...
\end{itemize}
  ...
  semantics shown in Appendix~\ref{appendix:bytecodetranslationdoc}).
...
the results can be seen in \namefig\ref{plot:TimeInStages}. 
This shows that the code generation is by far the most time consuming part of the pipeline. 
There is then a comparison of type inference between GHC and JVHC 
showing only a $\times 2$ decrease in the runtime cost of JVHC's TI 
...
\begin{table}
  ...
  \end{tabular}
  \caption{Regression values for code size form data in \namefig\ref{plot:codeSize} to $y=a x+b$}
  \label{table:codeSize}
\end{table}
\namefig\ref{plot:fibTiming} contains a comparison of the runtime of the \texttt{fib}$n$ program where 
$n$ ranges between $[1,35]$ showing that the asymptotic runtime of the code output
by each compiler has the same asymptotic complexity.
  ...
  \nametable\ref{table:fibCoef} shows
...
\begin{tabular}{c | c}
  ...
  Java & $1.62 \times 10^{-9}$
\end{tabular}
\caption{Coefficients from fitting the data in \namefig\ref{plot:fibTiming} 
...
coefficients in \nametable\ref{table:inlineCoef}, this shows a $12 \times$ speed up.
...
shown in \namefig\ref{plot:stackSize} and the maximum 
memory usage in \namefig\ref{plot:memUsed}. 
...
and a about a $2\times$ reduction maximum call stack height. 
...
reduce memory usage, and if less objects are allocated, less garbage collection cycles.
This data was collected for max stack height by generating a call stack at any 
point in the computation that could be a maximum thunk depth.
It could be a maximum thunk depth at any leaf in the \texttt{CoreExpr} tree
(\texttt{Var} or \texttt{Lit}). The JVM flag \texttt{MaxJavaStackTraceDepth} was used to increase
the stack height to a value greater than the maximum value returns from running this test (about $10^{4}$).
...
\begin{figure}
  ...
  \label{plot:memUsed}
\end{figure}
Functional inlining, as mentioned earlier, is not always a good idea.
...
\section{Usability}
There are three  on the programmer that this compiler introduces:
\begin{itemize}
...
Usability is hard to measure analytically, so an analytic measure that can be used is extra code size
...
without the need for all surrounding the boiler plate code.
...
function $f$ exposed from Haskell. $f_J$ will uncurry
