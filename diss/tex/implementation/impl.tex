...
no slack time was allocated to this project it would have been delivered only one week early. The code generation section took a little
...
to me not fully understanding the types used in JVM bytecode.
...
The Haskell grammar accepted by JVHC will possible to express using an
...
that cannot be known at compile time, hence a safe to over approximation is
made instead.
When counting the number of times a variable is free in an expression, 
it suffices to count each time to variable name exists in the tree, until
it is named in a $\lambda$ function at which point return the count for
than given path in the tree. The case expression can have multiple
...
$\unwrap{t}$ means, get the value from the thunk $t$. This would direct map to
calling \texttt{get} then implemented in JVM bytecode.
...
will map directly to a finite number of call-by-value 
transition relation steps, which will run in the same way the JVM does.
This is the JVM and the mapping run under eager (call-by-value)
semantics.
...
considered in turn. For each construct if will result in returning an
amount of bytecode which will be expected the parents of this node.
It is also possible that a separate \verb|.class| file will
be created in the case of a $\lambda$-function say. Then at parent
of a \verb|CoreExpr| tree will be the top level scope called \verb|Env|.
This will contain references to all top level functions
...
languages and will expose the top level functions
defined in the source file. I will not go over 
...
In Haskell terms all a monoid means is that the type exports two functions
\verb|mempty| and \verb|<>|, the first return an empty value and then 
second takes two variables of type \verb|Code| and combines them.
Variables with type \verb|Code| maps closely to JVM bytecode instructions.
For example there is a function \verb|new| which takes a Java type 
and returns a variable
of type \verb|Code|, this is very similar to the JVM bytecode instruction
\verb|new|. The JVM \verb|new| instructions create an object of the type
passed as an operand to this bytecode instruction. 
...
\end{JVMLst}
This continent Haskell interface and control flow handling 
are main the features that I used from this library.
...
loaded bytecode. The verifier will check things such as:
\begin{itemize}
...
which didn't conform to some of all of these verification conditions.
Since each \verb|CoreExpr| construct generates a small piece of
code $C$ this meant it was hard a first to made sure
that no $C$ would add or remove too many operands. It was very
helpful that when the \verb|.class| file was loaded the verifier
would give immediate feedback. The verifier also checks that
after each branch the type of each element on the stack
are the same, this again was very helpful when debugging \verb|case|
statements. This means I can be sure that each branch of a case statement
will return a stack with each element of the same type (or sharing a 
common parent type).
