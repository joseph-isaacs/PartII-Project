...
A work package involved either implemention one, or part of a pipeline
stage depending on an estimation of how long each stage would
take to implement.
...
\cref{figure:ganttchar} shows how to project progressed. The first
...
understand the bytecode semantics. In hindsight a better understanding
...
understanding in more detail the JVM bytecode specification.
...
let x = e' in e 
\end{HaskellLst}
All substituted expression must first be $\alpha$-renamed 
using fresh variables (or type variables) which don't exist 
anywhere in the program. This is done by using a ASCII which character, 
which is not allowed in either function or type names, \texttt{*}
was used.
...
This is more easily solved than the second problem as
I simply don't inline any recursive functions.
When trying to answer the question of whether the inlined
code will perform better, there is a lots of subtlety. 
...
During the preparation phase I created a document describe the 
...
in which case it will be thunk when evaluated will return the built in
...
be better is configuration file was used, or even better
...
these two maybe different). The I/O monad can only be run by the runtime
