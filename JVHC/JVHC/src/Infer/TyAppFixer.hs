...
type TyFxr e r = Subst -> [(CoreExpr,Type)] -> e -> FXR r
...
frxExpr sub mp (App e1 e2,t) =
  do e1t         <- lookupF e1 mp
     ...
     e1' <- frxExpr sub mp (e1,subFun1 e1t)
     e2' <- frxExpr sub mp (e2,subFun2 e2t)
     let e1'' = if ground e1' then applyTypes (subFun1 (map TVar e1LeftTv)) e1'
                              else e1'
         e2'' = if ground e2' then applyTypes (subFun2 (map TVar e2tv)) e2'
                              else e2'
     return (App e1'' e2'')



frxExpr sub mp (Lam v@(MkVar { varName = n, varType = TScheme [] lt}) e,t) =
  do (fn,body) <- splitAp t
     subst <- match fn lt
     let v' = MkVar { varName = n, varType = TScheme [] (apply sub lt) }
     e' <- frxExpr sub ((Var n,fn) : mp) (e,body)
     return (Lam v' e')

frxExpr sub mp (Lam v e,t) =
  do e' <- frxExpr sub mp (e,t)
     return (Lam v e')

frxExpr sub mp (l@(Lam _ _),t) =
  fail $ "scheme was quantified " ++ (show l)

frxExpr sub mp ((Let b e),t) =
  do bound <- getBound
     (b'@(ExprDef bi be)) <- frxExprDef sub mp b
     setBound bound
     let (MkVar { varName = n, varType = TScheme a st }) = bi
     e' <- frxExpr sub ((Var n,apply sub st):mp) (e,t)
     return (Let  b' e')

frxExpr sub mp ((Case e ct a),t) =
  do subst <- match ct t
     e1t <- lookupF e mp
     let e1t' = apply sub (apply subst e1t)
     let sub' = sub @@ subst
     e' <- frxExpr sub' mp (e,e1t')
     alts <- frxAlts sub' mp (a,t)
     return (Case e' t alts)

frxExpr _ _ (x,t) = return x

frxAlts :: TyFxr ([Alt Binder],Type) [Alt Binder]
frxAlts sub mp (alts,t) =
  mapM (\x -> frxAlt sub mp (x,t)) alts


frxAlt :: TyFxr (Alt Binder,Type) (Alt Binder)

frxAlt sub mp ((dc,bs,exp),t) =
  do (dc',subst) <- frxAltCon sub mp (dc,t)
     let sub' = sub @@ subst
         bs' = map (apply sub') bs
         nv  = map (\(MkVar { varName = n, varType = TScheme [] t }) -> (Var n,t)) bs'
     e' <- frxExpr sub' (nv ++ mp) (exp,t)
     return (dc',bs',e')

frxAltCon :: TyFxr (AltCon,Type) (AltCon,Subst)
frxAltCon sub mp ((DataAlt (MkDataCon { dName = dname, conType = TScheme [] ty })),t) =
  do (sc@(TScheme b ts),subst) <- quantifyTScheme (apply sub ty)
     let dc = DataAlt $  MkDataCon { dName = dname, conType = sc }
         sub' = sub @@ subst
     return (dc,sub')

frxAltCon _ _ (dc,_) = return (dc,[])
...
frxExprDef sub mp (ExprDef v@(MkVar {varName = n, varType = TScheme [] t}) e) =
  do let t' = apply sub t
     (sc@(TScheme b ts),subst) <- quantifyTScheme t'
     let var = MkVar { varName = n, varType = sc }
     let e' = addBigLambda b e
     e'' <- frxExpr (sub ++ subst) mp (e',ts)
